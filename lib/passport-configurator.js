...
PassportConfigurator.prototype.setupModels = function (options) {
  ...
  // Set up relations
  this.userModel =
    options.userModel || loopback.getModelByType(this.app.models.User);
  this.userCredentialModel =
    options.userCredentialModel || loopback.getModelByType(this.app.models.UserCredential);
  this.userIdentityModel =
    options.userIdentityModel || loopback.getModelByType(this.app.models.UserIdentity);
...
PassportConfigurator.prototype.configureProvider = function (name, options) {
  ...
  var noRedirectUri = options.noRedirectUri;
  var callbackURL = options.callbackURL;
  var authPath = options.authPath || ((link ? '/link/' : '/auth/') + name);
  var callbackPath = options.callbackPath || ((link ? '/link/' :
    '/auth/') + name + '/callback');
  var tokenExchangePath = options.tokenExchangePath || ((link ? '/link/' :
    '/auth/') + name + '/tokenExchange');
  var successRedirect = options.successRedirect || (link ? '/link/account' :
    '/auth/account');
  var failureRedirect = options.failureRedirect || (link ? '/link.html' :
    '/login.html');
  ...
  var storeUserProfile = options.storeUserProfile ||
    function (req, tokens, profile, done) {
      if (!tokens.hasOwnProperty('access_token')) {
        var e = new Error('Google OAuth Error');
        e.status = 401;
        e.message = 'Google Access Token not present';
        done(e);
      }
      if (link) {
        if (req.user) {
    ...
    }//end of function storeUserProfile
  ...
  switch (authType) {
      ...
      passport.use(name, new AuthStrategy(_.defaults({
        ...
        function (req, token, tokenSecret, profile, done) {
          if (link) {
            ...
            self.userIdentityModel.login(name, authScheme, profile,
              {token: token, tokenSecret: tokenSecret}, options,
    ...
    case 'openid connect':
        ...
        function (req, accessToken, refreshToken, profile, done) {
          if (link) {
            if (req.user) {
              self.userCredentialModel.link(req.user.id, name, authScheme,
                profile,
                {accessToken: accessToken, refreshToken: refreshToken}, options,
                done);
            } else {
              done('No user is logged in');
            }
      ...
      break;
    case 'oauth 2.0 authcode':
      passport.use(name, new AuthStrategy(_.defaults({
          clientID: clientID,
          clientSecret: clientSecret,
          apiKey: apiKey,
          noRedirectUri: noRedirectUri,
          passReqToCallback: true
        }, options), storeUserProfile));
      break;
    default:
      passport.use(name, new AuthStrategy(_.defaults({
          clientID: clientID,
   ...
   */
  if (authType === 'local') {
    self.app.post(authPath,
      passport.authenticate(name, options.fn || _.defaults({
    ...
    self.app.get(authPath, passport.authorize(name,
      _.defaults({scope: scope, session: session}, options.authOptions)));
  } else {
    self.app.get(authPath, passport.authenticate(name,
      _.defaults({scope: scope, session: session}, options.authOptions)));
  }
  ...
  if (link) {
    ...
    if (tokenExchangePath) {
      self.app.post(tokenExchangePath, jsonParser, customCallback);
    }
  }
};
