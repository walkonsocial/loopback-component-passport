...
PassportConfigurator.prototype.configureProvider = function (name, options) {
  ...
  var tokenExchangePath = options.tokenExchangePath || ((link ? '/link/' : '/auth/') + name + '/tokenExchange');
            ...
            passport.use(name, new AuthStrategy(_.defaults({
                ...
                function (req, tokens, profile, done) {
                    if (link) {
                        if (req.user) {
                            self.userCredentialModel.link(req.user.id, name, authScheme, profile,
                                {accessToken: tokens.access_token, refreshToken: tokens.refresh_token}, options, done);
                        } else {
                            done('No user is logged in');
                        }
                    } else {
